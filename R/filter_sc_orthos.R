...
dim(dat.all) #6284 ortholog groups occur in all 4 sym genomes
...
counts$Sgor <- apply(dat.all,1,function(x) str_count(x[4],"SymbC1.Gene_Models.PEP"))
counts$Skaw <- apply(dat.all,1,function(x) str_count(x[4],"SymbF.Gene_Models.PEP"))
